import { skillGroups, skillLegend } from "@/data/portfolio";
...
export function Skills() {
  return (
    <PortfolioSection id="skills" labelledBy="skills-heading">
      <SectionHeading
        id="skills-heading"
        ...
        {skillLegend.map((entry) => (
          <p key={entry.level} className="flex items-center gap-2 text-xs">
            <span
              ...
              aria-label={`Skill level ${entry.level}`}
            >
              {entry.level}
            </span>
              ...
              {group.items.map((item) => (
                ...
                >
                  ...
                  <span
                    ...
                    aria-label={`${item.name} level ${item.level}`}
                  >
                    {item.level}
                  </span>
                </li>
